...
camera/picking_session.py(vp-run-gt実行中の自然なアプローチ軌道をそのまま記録する)だけでは、
...
# lib/はscripts/dataset/直下(このファイルの1つ上)にあり、wrist_camera/自身の親を
# sys.pathに加えないとfrom lib...が解決できない。
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))
from lib.frame_recording import FrameWriter  # noqa: E402
...
# 大きすぎると物体が画角外に出て記録されるフレームが減り、小さすぎるとcamera/picking_session.pyの
...
def main() -> None:
    args = parse_args()
    rclpy.init()
    node = rclpy.create_node("jitter_dataset_collector")
