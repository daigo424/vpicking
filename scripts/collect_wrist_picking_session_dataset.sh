#!/usr/bin/env bash
# vp-run-gtでキューブをランダム位置に置きながらpicking_session.py(手先カメラ向け)で
# 学習用データを記録する。collect_picking_session_dataset.sh(俯瞰カメラ向け)と
# ほぼ同じ構成だが、CAMERA_NAMESPACE=wrist_cameraを指定して手先カメラのトピック・frame_idを
# 使う点、出力先を分ける点が異なる。
#
# 手先カメラは物体位置ではなくグリッパー(panda_hand)からの相対位置で見えるため、
# 俯瞰カメラのFOVに合わせて絞ったX/Y範囲を使う必要はなく、既存のvp-run-gtで
# 実績のある範囲をそのまま流用する。
#
...
for i in $(seq 0 $((ITERATIONS - 1))); do
    ...
    sleep 3
    docker exec -d edge_container bash -c "export CAMERA_NAMESPACE=wrist_camera; source /workspace/install/setup.bash 2>/dev/null; cd /workspace && pixi run picking_session --output-dir $OUTPUT_DIR --max-frames $FRAMES_PER_RUN --start-index $START_INDEX --min-interval-sec 0.15" >/dev/null 2>&1
    sleep 1
